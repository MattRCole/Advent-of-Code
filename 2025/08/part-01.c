...
#define JUNCTIONS_TO_CONNECT 1000
...
int main() {
    ...
    assert(junctionBoxes.length >= JUNCTIONS_TO_CONNECT, "Did not parse enough junction boxes! Junction boxes: %zu, to connect: %d\n", junctionBoxes.length, JUNCTIONS_TO_CONNECT);
    ...
    for(size_t i = 0; i < JUNCTIONS_TO_CONNECT && i < junctionBoxes.length; i++) {
