...
export const areAdjacentPoints = (pointA: Point, pointB: Point, options?: { cardinalOnly?: boolean, diagonalOnly?: boolean }) => {
  const {
    cardinalOnly = false,
    diagonalOnly = false,
  } = (options || {})
 ...
 */
export const getDirectionTraveled = (startingPoint: Point, endingPoint: Point): Direction => {
  const denormalizedDelta = { x: endingPoint.x - startingPoint.x, y: endingPoint.y - startingPoint.y }
  const delta: DeltaPoint = { x: denormalizedDelta.x && (denormalizedDelta.x > 0 ? 1 : -1), y: denormalizedDelta.y && (denormalizedDelta.y > 0 ? 1 : -1 )}
