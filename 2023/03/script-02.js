...
/** @type {(item: string, index: number, line: string, rowIndex: string) => string | PartNumberPart} */
...
for (let row = 0; row < grid.length; row++) {
    for (let column = 0; column < grid[row].length; column++) {
        if (grid[row][column] !== gear) continue

        partNumberPairs = [...partNumberPairs, reduceNeighbors((acc, item) => {
...
console.log(partNumberPairs.filter(r => r.length === 2).reduce((acc, ratios) => acc + (ratios[0].num * ratios[1].num), 0))
